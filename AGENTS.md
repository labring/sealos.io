...
- **Stack**: Keep the existing Next.js App Router, React, Fumadocs, pnpm, static
...
- pnpm 10.28.2 - `pnpm-lock.yaml` is present and CI uses frozen installs in all deployment workflows and `Dockerfile`.
...
- TypeScript compiler check - `pnpm lint` runs `rm -f tsconfig.tsbuildinfo && tsc --noEmit` from `package.json`.
- Dedicated test runner: Not detected in `package.json`; no Jest/Vitest/Playwright config file is present.
- Next.js CLI - `pnpm dev`, `pnpm build`, `pnpm build:analyze`, and `pnpm start` are defined in `package.json`.
- Fumadocs MDX generator - `postinstall` runs `fumadocs-mdx` from `package.json` to prepare content sources.
- `scripts/generate-apps-api.js` - Fetches Sealos template catalog data and produces local app listing assets before every build via `pnpm generate-apps`.
- `scripts/normalize-root-locale.js` - Runs after `next build` in `package.json` to normalize root locale output.
- `scripts/replace-image-paths.sh` - Runs during Docker builds to replace relative image paths with CDN URLs before `pnpm build` in `Dockerfile`.
- `@next/bundle-analyzer` 15.4.1 - Loaded dynamically when `ANALYZE=true` in `next.config.mjs` and invoked through `pnpm build:analyze`.
...
- Use pnpm with `pnpm-lock.yaml`; install native build toolchain when building `canvas` or `sharp` outside prebuilt-compatible environments.
- Run `pnpm generate-apps` before builds or use `pnpm build`, which runs it automatically.
- Use `pnpm lint` for TypeScript validation.
...
- Triggers: `pnpm build`, `pnpm dev`, `pnpm generate-apps`.
