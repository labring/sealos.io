...
assert.ok(collision, 'the source collection must contain a collision group');
...
async function readSitemap() {
  if (isRemote) {
    ...
    assert.equal(
      response.status,
      200,
      'AI Quick Reference sitemap must return 200',
    );
...
assert.equal(
  sitemapUrls.length,
  pages.length,
  'AI Quick Reference sitemap count must match source count',
);
assert.equal(
  new Set(sitemapUrls).size,
  sitemapUrls.length,
  'AI Quick Reference sitemap URLs must be unique',
);
...
for (const page of samplePages) {
  ...
  assert.ok(h1Match, `${pathname} must render an H1`);
  assert.ok(canonicalMatch, `${pathname} must render a canonical link`);
  assert.ok(titleMatch, `${pathname} must render a title`);
...
for (const pathname of [
  ...
  assert.ok(
    ...
    `${pathname} must remain unresolved`,
  );
}

console.log(
  `Verified sitemap (${sitemapUrls.length} URLs), ${samplePages.length} collision pages, and unresolved normalized/unknown routes at ${target}.`,
);
