...
const FooterLink: React.FC<FooterLinkProps> = ({
  ...
  <Link
    href={href}
    className={`font-medium text-zinc-400 transition-colors hover:text-blue-600 hover:underline hover:underline-offset-4 ${className || ''}`}
  >
    {children}
  </Link>
);
...
const Footer = async ({ lang = 'en' }: FooterProps) => {
  ...
  return (
    <>
          {/* 主体内容 - Footer 的链接部分 */}
      <div className="relative w-full pt-0 z-99">
        {/* 底部遮罩 */}
        <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-full w-screen bg-gradient-to-t from-black to-transparent pointer-events-none z-0 bg-black "></div>
        <div className="relative z-10 container flex flex-col lg:flex-row items-start justify-between px-0 text-sm gap-10 lg:gap-0 ">
          <div className="flex w-full flex-col items-start">
            <h2 className="mb-4 text-[32px] leading-[48px] font-medium">
              <div style={{ color: 'white' }}>Ready to Stop Configuring and</div>
              <div
                style={{
            ...
            <p
              style={{ color: '#A1A1AA', fontSize: '16px', marginBottom: '24px' }}
            >
          ...
          {/* Footer Links */}
          <div className="mt-10 w-full grid grid-cols-4 gap-6 items-start text-left lg:mt-0 lg:grid-cols-4 lg:gap-10">
            {[
        ...
        <div className="container grid grid-cols-1 grid-rows-3 items-center px-0 py-4 text-sm border-t border-gray-600 mt-[30px] sm:grid-cols-2 sm:grid-rows-2 lg:grid-cols-3 lg:grid-rows-1 z-99 relative">
          <div className="flex items-center gap-6 justify-self-center sm:justify-self-start col-start-1 row-start-1">
          ...
          <div className="text-center text-[#A1A1AA] col-start-1 row-start-2 sm:col-start-2 sm:row-start-1 sm:justify-self-end lg:col-start-2 lg:row-start-1">Copyright © 2025 Sealos. All rights reserved.</div>
          <div className="flex items-center gap-4 justify-self-center row-start-3 col-span-1 sm:row-start-2 sm:col-span-2 sm:justify-self-center lg:row-start-1 lg:col-start-3 lg:justify-self-end">
            <a
            ...
            <a
              ...
              title="X (Twitter)"
            >
              <img src="/images/og/icons/play.png" alt="Play" style={{ width: '16px', height: '16px' }} />
              </a>
            {lang === 'zh-cn' && (
              <>
                <a
