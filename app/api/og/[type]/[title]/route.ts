...
export async function GET(
  ...
  try {
    ...
    // Helper function to get color based on position in the grid - deterministic but with more randomness
    const getPositionalColor = (
      row: number,
      col: number,
      isBase1: boolean,
      triangleType: number,
      ...
      const xRatio = (col * triangleSize) / width;
      const yRatio = (row * triangleHeight) / height;
      ...
      // while still roughly following the gradient
    ...
    for (let row = -1; row < rows; row++) {
      ...
      for (let col = -1; col < cols; col++) {
        ...
        ctx.fillStyle = getPositionalColor(row, col, colorIndex1 % 2 === 0, 1);
        ...
        ctx.fillStyle = getPositionalColor(row, col, colorIndex2 % 2 === 0, 2);
    ...
    const tagWidth = ctx.measureText(tagText).width + 70; // Add padding
    const tagHeight = 45;
    ...
    try {
      ...
      ctx.fillText(tagText, centerX + textOffset, tagY + tagHeight / 2);
    } catch (e) {
      ...
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(tagText, centerX, tagY + tagHeight / 2);
    }
    ...
    drawWrappedText(ctx, decodedTitle, centerX, titleY, maxTitleWidth, 70);
