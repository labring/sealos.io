...
![Sealos 对象存储](./object-storage/images/sealos-objectstorage.zh-cn.png)

在对象存储应用界面中，点击【创建存储桶】按钮，输入存储桶名称，设置存储桶的访问权限，然后点击右上角的【应用】按钮。

![Sealos 对象存储控制台创建存储桶操作界面](./object-storage/images/object-storage-create-bucket-interface.zh-cn.png)
...
![Sealos 对象存储上传文件操作界面](./object-storage/images/object-storage-upload-file-interface.zh-cn.png)
...
<div className='fd-steps [&_h4]:fd-step'>
    ...
    ![Sealos 存储桶权限设置入口](./object-storage/images/sealos-bucket-permission-setting-entry.zh-cn.png)
...
![Sealos 对象存储访问密钥获取入口](./object-storage/images/sealos-objectstorage-access-key-get-entry.zh-cn.png)
...
![Sealos 对象存储访问密钥获取结果](./object-storage/images/sealos-objectstorage-access-key-get-result.zh-cn.png)
...
<div className='fd-steps [&_h4]:fd-step'>
    ...
    ![启用 Sealos 对象存储静态托管功能](./object-storage/images/sealos-objectstorage-static-hosting-enable.zh-cn.png)
    ...
    ![查看静态网站访问地址](./object-storage/images/sealos-objectstorage-static-hosting-access.zh-cn.png)

    <h4>配置自定义域名</h4>

    1. 在存储桶右上角点击三个点，选择【自定义域名】，会直接跳转到【应用管理】的应用变更界面
    2. 接下来的操作和【应用管理】的自定义域名配置一致，请参考应用管理的[自定义域名配置](/docs/guides/app-deploy/add-a-domain/)

    ![Sealos 对象存储自定义域名配置](./object-storage/images/sealos-objectstorage-static-hosting-custom-domain.zh-cn.png)
...
- [已创建并配置好特定语言环境的 Sealos DevBox 项目](/docs/guides/devbox/create-a-project)
