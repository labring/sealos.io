...
The first-upload signal is visual and stays inside the console. Open the
bucket's file list and confirm that the file you just uploaded is listed with
its file name, its size, and the upload timestamp you expect.

Seeing the file row in the list is enough to confirm success for this guide.
The bucket is private, so there is no public URL to check, and no command or
SDK call is needed at this point. Phase 15 will extend this section with
explicit failure checks; for now, a clean entry in the file list is the
...
If you read Chinese, the Chinese version of this page covers the same
first-upload flow and is kept in sync as the product evolves.
