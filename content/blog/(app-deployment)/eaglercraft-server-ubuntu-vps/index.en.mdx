...
It covers the bare-metal path the upstream project leaves open, since its README documents the Docker deployment. Every command, version, and measurement below comes from one recorded run on September 16, 2026; the sections that follow mark what that run did not exercise.
...
Opens the Eaglercraft template in a new tab if you would rather start from a managed deployment.
...
One Java process serves the browser client page and the game WebSocket on the same port, which is why port `5200` handles both. The Server Management Panel is a separate Python process on `5201`, and it talks to the game server over RCON on `127.0.0.1:25575`.
...
- An Ubuntu 22.04 or 24.04 server with `sudo`, and root or an equivalent account. The recorded run used Ubuntu 24.04.4 LTS with kernel `7.0.0-31-generic`.
...
Generate the RCON password rather than typing one, and reuse it verbatim in `server.properties`:
...
One detail in that directory exists for a specific reason. The `eaglercraft` account has `/usr/sbin/nologin` as its shell, and the game server runs inside tmux, so `bin/tmux.conf` pins the shell tmux uses to spawn panes and keeps a crashed pane addressable:
...
`bin/paper-tmux.sh` starts the tmux server with `-f bin/tmux.conf` for the same reason. `bin/backup.sh` and the panel then send keys to that running server over the socket path from `TMUX_TMPDIR`. Without the configuration the pane exits as soon as it spawns and the panel reports the game server as offline.
...
The `header_up` line carries the weight here. The shipped listener sets `forward_ip: true` together with `forward_ip_header: X-Real-IP`, so the gateway reads that header on every connection and drops a request that arrives without it. In the recorded run a request straight to `127.0.0.1:5200` without the header returned no response at all, and the same request through Caddy returned `200` with `2158` bytes and the title `EaglercraftX 1.8`.
...
The panel answers on `0.0.0.0:5201`, and its bind address has no environment override. Forward the port over SSH instead of publishing it:
