...
seoTitle: 'Host an Eaglercraft Server for Friends'
...
Open the template to follow the walkthrough below. Keep this guide open while you deploy.
...
For the full cost picture across free, paid, and self-managed Eaglercraft hosting, see [Eaglercraft Hosting Costs: Free Options, Paid Plans, and Setup Trade-offs](/blog/eaglercraft-hosting-cost/).
...
On the template page, choose **Deploy Eaglercraft**. In the deployment form, select `1.8` for `minecraft_version`, then set `rcon_password`: this is your **Administrator Password**. Keep it private and save it before continuing. Submit the configuration, then sign in when prompted.
...
Running the server yourself on a VPS is a documented path: [How to Host an Eaglercraft Server on an Ubuntu VPS](/blog/eaglercraft-server-ubuntu-vps/) walks through the runtime install, the systemd units, the Caddy HTTPS entry point, the admin panel over an SSH tunnel, and a rehearsed world restore.

If Docker Compose is already part of your workflow, [Run Eaglercraft with Docker Compose](/blog/eaglercraft-server-docker/) deploys the same stack from one image with a persistent bind mount, a loopback-only admin panel, and a rehearsed backup and restore.

Once you have checked the first player, your friend's connection, and the retained world, keep the Browser Play Link and your private credentials for the next session. Start your shared world with the Eaglercraft template:
