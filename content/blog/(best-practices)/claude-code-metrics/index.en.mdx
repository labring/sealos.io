---
title: 'Claude Code Metrics Dashboard: Grafana Setup (2026)'
imageTitle: 'Claude Code Metrics Dashboard'
description: 'Track Claude Code cost, tokens, sessions, and telemetry with Grafana. Includes Docker Compose and Sealos setup paths with OpenTelemetry.'
date: 2025-12-12
...
faq:
  ...
  - question: "Is my prompt content sent to the monitoring stack?"
    answer: |-
      Not by default. Claude Code telemetry exports **metadata first**—token counts, costs, timing, model names, and related attributes. The actual text of your prompts and Claude's responses is not included by default, and the `claude_code.user_prompt` event records prompt length rather than prompt text.

      There is one important caveat from Anthropic's monitoring docs: tool execution events can include **bash commands and file paths** in the `tool_parameters` field, which may still expose sensitive values if your commands contain secrets. If you send telemetry to a shared backend, plan to filter or redact sensitive fields before treating the stack as safe for broad team access.
...
| If you want to... | Best path | Time to first dashboard | Best next step |
|---|---|---|---|
| Keep everything local and tweak every config | **Docker Compose** | ~10-20 minutes | Jump to [Docker Compose Monitoring Setup](#docker-compose-monitoring-setup-step-by-step) |
| Share dashboards quickly without fighting SSL and public access | **Sealos App Store** | ~2-5 minutes | Jump to [Cloud Alternative: Deploying on Sealos](#cloud-alternative-deploying-on-sealos) |
| Track team usage with less ops overhead | **Sealos App Store** | ~2-5 minutes | [Deploy the Grafana OTEL stack](https://sealos.io/products/app-store/grafana-otel) |
...
| Metric | Why it matters first | What to do with it |
| --- | --- | --- |
...
| **Active time** | This helps distinguish "Claude is busy doing useful work" from "people are waiting on the tool or thrashing." | Compare active time against commits, PRs, and session count to see whether usage is productive or just noisy. |
...
| --- | --- | --- |
| Learn the stack and keep everything local | **Docker Compose** | Maximum control over Collector, Prometheus, and Grafana config |
| Share a dashboard quickly with a team | **Sealos App Store** | Public URL, SSL, and persistent storage without extra ingress work |
| Get cost visibility this afternoon instead of next week | **Sealos App Store** | Shortest path from zero to usable dashboard |

## Read This Before You Enable Telemetry
...
## Docker Compose vs. Sealos in Practice

| Question | Docker Compose | Sealos |
| --- | --- | --- |
| Do you want to learn every moving part locally? | **Best fit** | Fine, but overkill |
| Do you need a public Grafana URL with SSL quickly? | Manual work | **Best fit** |
| Do you want teammates to see the same dashboard today? | Possible, but ops-heavy | **Best fit** |
| Do you want persistent storage without hand-rolling infra? | Manual | **Best fit** |
| Do you need full control and local-only exposure? | **Best fit** | Less important |
...
## Why Sealos Is the Faster Team Path
...
Here is the practical problem this page solves: you can use Claude Code heavily for refactors, tests, and debugging, then get the bill later with no clear explanation of which workflows burned the budget. [Claude Code](https://docs.anthropic.com/en/docs/claude-code) telemetry already tracks the answer. You just need to export and visualize it.
...
- If you need a shareable dashboard now, deploy the [Grafana OTEL stack on Sealos](https://sealos.io/products/app-store/grafana-otel).
- If you also work away from your desk, read [Claude Code on Phone: Best Setup for iPhone and Android](/blog/claude-code-on-phone).
- If you want the platform context behind hosted dev environments, read [What Is DevBox?](/blog/what-is-devbox) and [What Is a Cloud Development Environment?](/blog/what-is-cloud-development-environment).
